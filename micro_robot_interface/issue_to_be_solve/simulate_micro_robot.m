...
if success == 1
    if incline_angle == 0
        ...
        if max_velocity > 0
            output = -1;                    % output '-1' if the robot went backwards and slipped off the incline
        else
            output = incline_angle;         % output the incline angle if the robot succeeded in climbing it
        end
    end
else
    output = -1;
